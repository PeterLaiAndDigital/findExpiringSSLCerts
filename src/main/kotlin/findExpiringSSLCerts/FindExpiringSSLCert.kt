    ...
    fun main() {
        ...
        println(
            "Terminated ${now()} \n " +
                    ...
                    "$listOfTimedOuts \n" +
                    "=========================== List of NOT expiring URLs +$minDaysBeforeWarning: ========================= \n" +
    ...
    private fun getSSLCertsUsingSockets(url: String) {
        try {
            ...
            for (c in socket.session.peerCertificates) { //For every cert in the chain
                ...
                if (daysLeft < minDaysBeforeWarning)
                    println("Certificate expires on : ${xc.notAfter} ($daysLeft days to go) : $url")
                else
